...
# It took about 6 min to solve the env on RTX 3080 Ti Laptop, i9-12900H
...
class DQNAgent:
    def __init__(
        ...
        # tau를 초기에 크게 시작해서 학습하면서 점차 줄여가는 구조
        tau_init = 2.0
        tau_final = 0.5
    ...
    def anneal_tau(self):
        # step_count에 따라 tau 값을 점진적으로 감소
        ratio = min(1.0, self.step_counter / self.anneal_steps)
        self.tau = self.tau_init + ratio * (self.tau_final - self.tau_init)
    ...
    def train(self):
        ...
        # Next action from policy network
        next_action = self.policy_net(next_states).argmax(1).unsqueeze(1)

        # Next Q Values using next action: Q_target(s', argmax_a' Q(s',a'))
        next_Q = self.target_model(next_states).gather(1, next_action).squeeze().detach()  # [64]
